// "Edit Product" modal, opened from ListProduct.jsx's pencil icon.
// Broader than BrandEditModal/CategoryEditModal since a product has two
// collections (features, images) alongside its plain fields - those are
// added/removed immediately against the backend (each is its own row),
// while name/price/stock/descriptions are staged and saved together via
// the "Save Changes" button, same as the Brand/Category pattern.
import {
  Package,
  FileText,
  X,
        ...
        {loadingEditDetail ? (
          ...
          <>
            ...
            <div className="p-6 space-y-6 max-h-[65vh] overflow-y-auto">
              ...
              {/* Specifications - attributes mapped to this product's
                  category (Condition, RAM, etc.), same as AddProduct.jsx.
                  Reflects the category the product had when this modal was
                  opened - switching Category above won't refresh this list. */}
              {editCatAttributes.length > 0 && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Specifications
                  </label>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    {editCatAttributes.map((attr) => (
                      <div key={attr.attribute_id}>
                        ...
                        <input
                          type="text"
                          value={editAttributeValues[attr.attribute_id]?.value || ""}
                          onChange={(e) =>
                            handleEditAttributeChange(attr.attribute_id, e.target.value)
                          }
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    min="0"
                    step="0.01"
                    value={editData.product_current_price}
                    onChange={(e) =>
                      setEditData({
                        ...editData,
                        product_current_price: e.target.value,
                      })
                    }
                    className={inputClass}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    min="0"
                    step="0.01"
                    value={editData.product_discounted_price}
                    onChange={(e) =>
                      setEditData({
                        ...editData,
                        product_discounted_price: e.target.value,
                      })
                    }
                    className={inputClass}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    min="0"
                    step="1"
                    value={editData.product_stock_quantity}
                    onChange={(e) =>
                      setEditData({
                        ...editData,
                        product_stock_quantity: e.target.value,
                      })
                    }
                    className={inputClass}
                  />
